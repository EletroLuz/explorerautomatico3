...
local function check_walkable_area()
    ...
    local check_radius = 25 -- Check radius in meters
    local min_z = 10
    local max_z = 20
    local representative_z = player_pos:z() -- Use player's current z as the representative z
    ...
    for x = -check_radius, check_radius, grid_size do
        for y = -check_radius, check_radius, grid_size do
            local point_to_mark = nil
            for z = min_z, max_z, grid_size do
                ...
                if utility.is_point_walkeable(point) then
                    point_to_mark = vec3:new(
                        player_pos:x() + x,
                        player_pos:y() + y,
                        representative_z
                    )
                    break -- Stop checking further z-values once a walkable point is found
                end
            end
            if point_to_mark then
                if is_point_in_explored_area(point_to_mark) then
                    graphics.text_3d("explored", point_to_mark, 15, color_white(128))
                else
                    graphics.text_3d("unexplored", point_to_mark, 15, color_green(255))
                end
            end
        end
    end
